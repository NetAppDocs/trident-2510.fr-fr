...
En règle générale, vous devez effectuer la mise à niveau en utilisant la même méthode que celle utilisée lors de l'installation initiale, cependant vous pouvez link:../trident-install/kubernetes-deploy.html#moving-between-installation-methods["passer d'une méthode d'installation à une autre"]. Il existe deux options pour mettre à niveau Trident.
